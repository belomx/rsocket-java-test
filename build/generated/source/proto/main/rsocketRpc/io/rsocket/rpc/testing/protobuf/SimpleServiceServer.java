    ...
    try {
      ...
      switch(io.rsocket.rpc.frames.Metadata.getMethod(metadata)) {
        case SimpleService.METHOD_REQUEST_REPLY: {
          ...
          return service.requestReply(io.rsocket.rpc.testing.protobuf.SimpleRequest.parseFrom(is), metadata).map(serializer).transform(requestReply).transform(requestReplyTrace.apply(spanContext));
        }
        default: {
